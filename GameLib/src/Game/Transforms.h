...
namespace Game {
	...
	struct ChainBase
	{
		virtual constexpr vec3 Go(const vec3&, const State<S>&) const
		{
			return {};
		};
	};
	...
	{
		constexpr vec3 Go(const vec3& in, const State<S>& state) const override
		{
	...
	{
		constexpr vec3 Go(const vec3& in, const State<S>&) const override
		{
			return in;
		}
	};
